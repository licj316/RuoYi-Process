...
package com.ruoyi.process.core.plugin.flowable.delegate;
...
public class CustomClassDelegate extends ClassDelegate {
    ...
    public CustomClassDelegate(String id, String className, List<FieldDeclaration> fieldDeclarations, boolean triggerable, Expression skipExpression, List<MapExceptionEntry> mapExceptions) {
        super(id, className, fieldDeclarations, triggerable, skipExpression, mapExceptions);
    }

    public CustomClassDelegate(String className, List<FieldDeclaration> fieldDeclarations) {
    ...
    protected ActivityBehavior getActivityBehaviorInstance() {
        ...
        if (className.startsWith("$ioc:")) {
            try {
                delegateInstance = SpringUtils.getBean(className.substring(5));
            } catch (Exception e) {
